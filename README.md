...
### 1. 启动本地网页端体验 (1 真人 vs 2 AI)
执行以下命令直接启动本地 Web 游戏服务：
```bash
mvn exec:java
```
服务启动后，在浏览器中访问：
...
#### 网页端特色体验：
- **经典扑克桌体验**：实时渲染手牌、桌面出牌气泡、剩余牌量以及轮到哪家出牌的高亮指示。
- **出牌/过牌交互**：点击手牌自动弹起选中，严格根据跑得快“能管必管”规则校验。
- **💡 AI 智能提示**：遇到疑难牌局时，点击“AI 提示”按钮，后台将通过 PIMC MCTS 算法自动计算出最佳着法并高亮选中。
- **🔍 实时战术雷达**：右侧面板实时展示对手 AI 的深度思考链路（包括不同世界采样中的候选动作分布、访问次数 $N$ 与平均预估胜率）。
- **📊 记牌器**：清晰统计场上已打出和各家剩余的点数分布（3~2）。

---

### 2. 命令行 Benchmark 模拟演示
如果需要在终端查看 3 个 AI 纯自动对局的思考日志与结算：
```bash
